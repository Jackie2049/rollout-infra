...
> 2026-06-07 | 综合8×RTX 4090 PCIe集群所有benchmark实验的最终决策指南
...
│                                                                      │
│ 单GPU推理                                                             │
│   → 最优方案! 7B FP16 fits 24GB(14GB权重+10GB KV)                  │
│   → vLLM OPT-125M: 峰值30K tok/s@B=512                               │
│   → decode严重memory-bound: 0.75 TFLOPS(0.45%peak)                │
│   → FlashAttention: 省85-97%内存(防OOM) 但decode更慢0.67-0.84x    │
│                                                                      │
│ 多GPU推理(TP)                                                          │
...
│                                                                      │
│ torch.compile: forward 3.75x, training 1.31x                        │
│   → GRPO仅1.39x(rollout 74%不可编译)                                │
│   → 解决: vLLM/SGLang async rollout + compile训练部分               │
│                                                                      │
...
推理:
  7B FP16: 14GB fits 24GB → 单GPU最优
  decode: 0.75 TFLOPS(0.45%peak) → 严重memory-bound
  FlashAttention: 省85-97%内存, decode更慢0.67-0.84x
  TP PCIe: 不可行(通信96%, 无NVLink)
  Prefix Sharing: full-model 2.46x, attn-only 0.99x(MLP 82%主导)
...
优化:
  ...
  compile: forward 3.75x, training 1.31x, GRPO 1.39x
...
推理benchmark:
  ...
  tools/benchmark_paged_attention_4090.py — Paged Attention overhead
