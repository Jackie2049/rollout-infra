#!/usr/bin/env python3
"""Proper GRPO σ-normalization vs No-normalization Comparison — RTX 4090

Compare GRPO with proper rollout (same prompt, n completions):
1. SFT warm-start (200 steps) → baseline
2. SFT → GRPO normalized (A=(r-μ)/σ, standard GRPO)
3. SFT → GRPO unnormalized (A=r-μ, no σ division)
4. SFT → GRPO normalized + gradient noise (σ=0.01)

Uses autoregressive rollout from mini_grpo_training.py for proper group comparison.

Usage:
  CUDA_VISIBLE_DEVICES=0 python -u tools/proper_grpo_sigma_comparison.py
"""
...
def grpo_step(model, prompts, n_samples, max_response_len, optimizer,
              device, normalize_sigma=True, grad_noise_sigma=0.0):
    """One GRPO training step with optional σ-normalization and gradient noise."""
...
def train_grpo(model, optimizer, device, num_steps, n_samples=4,
               max_response_len=3, normalize_sigma=True, grad_noise_sigma=0.0):
    """GRPO training loop with proper rollout."""
    eval_trajectory = []
    step_metrics = []
    ...
    for step in range(num_steps):
        ...
        metrics = grpo_step(model, prompts, n_samples, max_response_len,
                            optimizer, device, normalize_sigma, grad_noise_sigma)
...
def main():
    ...
    print("=" * 70)
    print("Proper GRPO σ-normalization Comparison — RTX 4090")
    print("=" * 70)
    if torch.cuda.is_available():
        print(f"GPU: {torch.cuda.get_device_name(0)}")
    print(f"Model: {args.model_size}, SFT: {args.sft_steps} steps, GRPO: {args.grpo_steps} steps, n={args.n_samples}")
    ...
    strategies = [
        ('sft_then_grpo_normalized', True, 0.0, 'SFT→GRPO A=(r-μ)/σ'),
        ('sft_then_grpo_unnormalized', False, 0.0, 'SFT→GRPO A=r-μ'),
        ('sft_then_grpo_normalized_noise', True, 0.01, 'SFT→GRPO A=(r-μ)/σ + noise'),
    ]

    results = {}

    for key, normalize, noise_sigma, label in strategies:
        print(f"\n{'='*50}")
        ...
        print(f"[Phase 2] GRPO ({args.grpo_steps} steps, n={args.n_samples}, "
              f"normalize={normalize}, noise={noise_sigma})")
        grpo_metrics, grpo_eval = train_grpo(
            model, optimizer, device, args.grpo_steps, args.n_samples,
            normalize_sigma=normalize, grad_noise_sigma=noise_sigma)
    ...
    # Summary
    print("\n" + "=" * 70)
    print("Summary: σ-normalization Effect in Proper GRPO")
    print("=" * 70)

    for key, normalize, noise, label in strategies:
        r = results[key]
        print(f"  {label}: SFT→{r['sft_final_eval']:.1%}, GRPO→{r['grpo_final_eval']:.1%}, "
              f"peak={r['grpo_peak_eval']:.1%}, reward={r['avg_reward']:.3f}")

    norm_eval = results['sft_then_grpo_normalized']['grpo_final_eval']
    unnorm_eval = results['sft_then_grpo_unnormalized']['grpo_final_eval']
    print(f"\n  σ-normalization effect: {norm_eval:.1%} vs {unnorm_eval:.1%} "
          f"(Δ={norm_eval - unnorm_eval:+.1%})")
