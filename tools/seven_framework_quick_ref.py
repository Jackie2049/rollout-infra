#!/usr/bin/env python3
"""7-Framework API Quick Reference Generator.

Generates a markdown quick-reference table for the 7 key AI infra frameworks:
DeepSpeed, Megatron-LM, vLLM, verl, MindIE, rLLM, PyTorch

This is designed as a practical "personal advisor" tool — the key APIs,
config patterns, and one-liners you need to know for each framework.
"""

import os

OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "..", "notebook", "projects")

QUICK_REFS = {
    "DeepSpeed": {
        "description": "ZeRO distributed training — offload optimizer states to CPU/NVMe",
        "apis": [
        ...
        "memory_formula": "ZeRO-1: 4Ψ+(12+K)Ψ/N | ZeRO-3: (16+K)Ψ/N | ZeRO-Infinity: ≈2Ψ_layer(只持当前层!)",
        "rtx4090_tip": "7B ZeRO-2+CPU Adam+BF16 → 有效; ZeRO-3慢(offload开销); 单GPU用verl更好",
    },
    "Megatron-LM": {
        "description": "3D parallel training (TP+PP+DP) — NVIDIA industrial standard",
        "apis": [
        ...
        "memory_formula": "TP: Ψ/TP_size per GPU | PP: layers/stages | SP: activation/TP_size",
        "rtx4090_tip": "TP>1无意义(PCIe瓶颈); 单GPU用; MoE EP=1; PP不适合单GPU",
    },
    "vLLM": {
        "description": "General inference serving — PagedAttention + Continuous Batching",
        "apis": [
        ...
        "memory_formula": "KV Cache = 2 * num_layers * num_heads * head_dim * seq_len * batch_size * dtype_bytes / block_size",
        "rtx4090_tip": "7B INT4+INT8KV+GQA-8+FlashInfer → 4,791 tok/s; EAGLE → 9,088 tok/s",
    },
    "verl": {
        "description": "RL training engine — GRPO/PPO with Ray distributed",
        "apis": [
        ...
        "memory_formula": "7B GRPO+BF16 = 20.04GB fits RTX 4090; LoRA+CPU Adam → 进一步省内存",
        "rtx4090_tip": "7B GRPO单GPU可行; CPU Adam省50% opt内存; prefix grouper省30% KV",
    },
    "MindIE": {
        "description": "Huawei Ascend inference — CANN+ATB+HCCL (Ascend NPU专用)",
        "apis": [
        ...
        "memory_formula": "Ascend 910B: FP16=320 TFLOPS, 64GB HBM; RTX 4090: FP16=82.6 TFLOPS, 24GB",
        "rtx4090_tip": "MindIE不支持RTX 4090(昇腾专用); 用vLLM替代; 910B是企业级国产替代",
    },
    "rLLM": {
        "description": "Agentic RL orchestration — any harness, any backend, any benchmark",
        "apis": [
        ...
        "memory_formula": "GRPO = (r-mean)/(std+ε); RLOO = N/(N-1)*(r-mean); REINFORCE++ = centered/batch_std",
        "rtx4090_tip": "tinker backend单GPU可用; 需vLLM/SGLang推理后端; same code eval+train",
    },
    "PyTorch": {
        "description": "Foundation framework — distributed, compile, custom ops, FSDP2",
        "apis": [
        ...
        "memory_formula": "FSDP2: (16+K)Ψ/N (per-param DTensor); vs FSDP1: flat→需transform",
        "rtx4090_tip": "FSDP2+BF16+compile = 有效; custom_op注册fused kernel→不graph break",
    },
}


def generate_quick_ref():
    """Generate the 7-framework quick reference markdown."""
    lines = []
    lines.append("# 7框架API速查表: DeepSpeed / Megatron-LM / vLLM / verl / MindIE / rLLM / PyTorch")
    lines.append("")
    lines.append("> 2026-06-15 | 贴身顾问实用工具 — 关键API、配置模式、一行命令")
    lines.append("> 详细笔记见各框架 notebook/projects/ 下的阅读文件")
    lines.append("")
    lines.append("## 总览")
    lines.append("")
    lines.append("| 框架 | 类型 | 一行启动 | RTX 4090最优配置 |")
    lines.append("|------|------|----------|------------------|")

    for name, ref in QUICK_REFS.items():
        desc = ref["description"]
        tip = ref["rtx4090_tip"]
        # Find the most useful "one-liner" from apis
        first_api = ref["apis"][0][1].split("\n")[0][:50]
        lines.append(f"| {name} | {desc[:20]} | `{first_api}` | {tip[:40]} |")
    ...
    for name, ref in QUICK_REFS.items():
        lines.append(f"## {name}")
        lines.append("")
        lines.append(f"**定位**: {ref['description']}")
        lines.append("")
        lines.append(f"**内存公式**: `{ref['memory_formula']}`")
        lines.append("")
        lines.append(f"**RTX 4090建议**: {ref['rtx4090_tip']}")
        lines.append("")
        lines.append("| 用途 | API/配置 | 说明 |")
        lines.append("|------|----------|------|")
        for api_name, api_code, api_note in ref["apis"]:
            # Format code as inline code
            code_short = api_code[:60] + "..." if len(api_code) > 60 else api_code
            lines.append(f"| {api_name} | `{code_short}` | {api_note} |")
        lines.append("")
        lines.append("---")
        lines.append("")

    # Add cross-framework patterns
    lines.append("## 跨框架通用模式")
    lines.append("")
    lines.append("### 1. 分布式训练通用配置")
    lines.append("")
    lines.append("```python")
    lines.append("# 所有框架: 初始化分布式")
    lines.append("import torch.distributed as dist")
    lines.append("dist.init_process_group(backend='nccl')")
    lines.append("local_rank = int(os.environ['LOCAL_RANK'])")
    lines.append("torch.cuda.set_device(local_rank)")
    lines.append("```")
    lines.append("")
    lines.append("### 2. BF16训练通用模式")
    lines.append("")
    lines.append("```python")
    lines.append("# DeepSpeed: bf16.enabled=True in config")
    lines.append("# Megatron: --bf16 flag")
    lines.append("# PyTorch: torch.amp.autocast('cuda', dtype=torch.bfloat16)")
    lines.append("# verl: actor_rollout_ref.model.dtype=bfloat16")
    lines.append("```")
    lines.append("")
    lines.append("### 3. 梯度检查点通用模式")
    lines.append("")
    lines.append("```python")
    lines.append("# PyTorch: torch.utils.checkpoint.checkpoint(fn, *args)")
    lines.append("# DeepSpeed: activation_checkpointing.partition_activations=True")
    lines.append("# Megatron: --recompute-granularity selective")
    lines.append("# verl: actor.use_gradient_checkpointing=True")
    lines.append("```")
    lines.append("")
    lines.append("### 4. 推理服务通用模式")
    lines.append("")
    lines.append("```python")
    lines.append("# vLLM: vllm serve model_name")
    lines.append("# SGLang: python -m sglang.launch_server --model-path model_name")
    lines.append("# MindIE: mindie-service --config config.yaml")
    lines.append("# rLLM gateway: rllm-model-gateway --worker url")
    lines.append("```")
    lines.append("")
    lines.append("### 5. RL训练通用模式")
    lines.append("")
    lines.append("```python")
    lines.append("# verl: RayPPOTrainer(config) → train()")
    lines.append("# rLLM: AgentTrainer(workflow_class, backend='verl') → train()")
    lines.append("# GRPO: adv = (r - mean) / (std + ε)  ← verl/rLLM/Megatron共享")
    lines.append("```")

    output_path = os.path.join(OUTPUT_DIR, "seven-framework-api-quick-ref.md")
